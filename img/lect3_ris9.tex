...
% }, 		
   		...
   		\addplot[thick, domain={0:1*pi},samples=600] {sin(deg(x))};
   		\addplot[thick, domain={0:1*pi},samples=600] {-sin(deg(x))};
   		\addplot[domain={0:1*pi},samples=600] {sin(deg(x*30))*sin(deg(x))};

   		\draw[dashed] (0,0) circle  [x radius=0.5, y radius=1.5];
   		\draw[dashed] (9,0) circle  [x radius=0.5, y radius=1.5];

   		\draw[dashed] (0,1.5) -- (9,1.5) (0,-1.5) -- (9,-1.5);

   		\draw[->,thick,overdraw=2pt] (pi/2,1) -- (pi/2+1,1) node [right] {$v_\text{гр}$};

\contourlength{2pt};

   		\draw[->,thick,overdraw=3pt] (pi/3,0) -- (pi/3+0.5,0) node [right] {\contour{white}{$v_\text{ф}$}};

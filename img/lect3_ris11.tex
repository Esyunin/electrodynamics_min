...
\begin{tikzpicture}[z={(1cm,0cm)}, y={(-0.4cm,-0cm)}, x={(0cm,1cm)}, scale=1,
    ...
    \draw (1.2,0,4)--(1.2,0,0);
    % \draw[interface1]  (1.2,0,4)--(1.2,0,0);
    % \draw[interface1]  (-1.2,0,0)--(-1.2,0,4);
    % \draw[dashed] (1,0,0) -- ++(0,0,4);
    % \draw[dashed] (-1,0,0) -- ++(0,0,4);
    \draw (-1.2,0,0)--(-1.2,0,4);
    \begin{scope}[canvas is yx plane at z=0]
        \draw[fill=white,pattern=north east lines,pattern color=black!20] (0,0,0) node[sloped] {$\Sigma$} circle (1.2cm);% node[right, yshift=.8cm, xshift=.45cm] {$L$};
        % \draw[fill=white] (0,0,0) circle (1cm);
    \end{scope}
        \begin{scope}[canvas is yx plane at z=4]
        % \draw[dashed] (0,0,0) circle (1cm);

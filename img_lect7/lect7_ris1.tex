...
\begin{tikzpicture}[z={(1cm,0cm)}, y={(-0.4cm,-0cm)}, x={(0cm,1cm)}, scale=1,
    ...
    \begin{scope}[canvas is yx plane at z=1.3]
        \draw[pattern=north east lines,] (0,0,0) circle (1.2cm) node[right=1.5em, yshift=-1em] {$\Sigma$};
    \end{scope}
    ...
    \begin{scope}[canvas is xz plane at y=0]
        \draw[draw=none,fill=white] (-2,4) rectangle (2,3);
    ...
    \begin{scope}[canvas is yx plane at z=0]
        ...
        \draw[fill=white] (0,0,0) circle (1.2cm);
    \end{scope}
    \draw[thick] (O) -- +(0,  0,   0.5*1*0.9);

    % \draw (0.5,0,2) node {$\varepsilon,\mu=\mathrm{const}$};

    % \draw[->] (n1.east) to[out=0,in=180] (t1);
...
\begin{scope}[canvas is xz plane at y=0]
    \draw[->] (0.25,0) -- node[above,pos=0.35] {$\Pi$} ++(0,0.7);

    \draw (1.2,2.2)++(0.1,0) -- ++ (-0.2,0) coordinate (1);
    \draw (1.2,3)++(0.1,0) -- ++ (-0.2,0) coordinate (2);
    \draw[<->] (1) -- node[below] {$1$} node[above=1em] {$P_\text{ст}$} (2);
\end{scope}  
\end{tikzpicture}
\end{document}